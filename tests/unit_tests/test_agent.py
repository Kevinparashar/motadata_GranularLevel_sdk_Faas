...
class TestAgentAdvanced:
    ...
    @pytest.mark.asyncio
    async def test_attach_memory(self, agent):
    ...
    def test_attach_circuit_breaker(self, agent):
        ...
        assert agent.circuit_breaker_config == config
    ...
    def test_attach_circuit_breaker_default(self, agent):
        ...
        assert agent.circuit_breaker is not None
        assert agent.circuit_breaker_config is not None

    @pytest.mark.asyncio
    async def test_attach_health_check(self, agent):
        """Test attaching health check."""
        agent.attach_health_check()
        
        assert agent.health_check is not None
    ...
    async def test_receive_message_with_messages(self, agent):
        ...
        agent.message_queue = [message1, message2]
    ...
    def test_add_prompt_template(self, agent):
        """Test adding prompt template."""
        with patch("src.core.agno_agent_framework.agent.create_prompt_manager") as mock_create:
            mock_pm = MagicMock()
            mock_pm.add_template = Mock()
    ...
    def test_add_prompt_template_error(self, agent):
        ...
        with patch("src.core.agno_agent_framework.agent.create_prompt_manager", return_value=None):
            with pytest.raises(AgentConfigurationError, match="Prompt manager is not available"):
                agent.add_prompt_template(
                    name="test",
                    version="1.0",
                    content="Test"
    ...
    async def test_get_health_error_state(self, agent):
        ...
        assert health["status"] == "error" or health["status"] == "unhealthy"
    ...
    async def test_execute_task_with_agent_execution_error(self, agent):
        ...
        with patch.object(agent, "_execute_with_retry", new_callable=AsyncMock) as mock_execute:
            mock_execute.side_effect = AgentExecutionError(message="Execution error")
            
            with pytest.raises(AgentExecutionError):
                await agent.execute_task(task)
            
            assert agent.status == AgentStatus.IDLE  # Should reset in finally
    ...
    async def test_execute_task_with_agent_execution_error_handling(self, agent):
        ...
        with patch.object(agent, "_execute_with_retry", new_callable=AsyncMock) as mock_execute:
            ...
            # Status should be reset to IDLE in finally block
            assert agent.status == AgentStatus.IDLE
            assert agent.current_task is None
    ...
    def test_create_agent_from_state(self):
        ...
        from src.core.agno_agent_framework.agent import Agent
    ...
    async def test_load_state_full_flow(self):
        ...
        try:
            with patch("aiofiles.open") as mock_open, \
                 patch("src.core.agno_agent_framework.agent.PromptContextManager", MagicMock()):
                mock_file = AsyncMock()
                mock_file.read = AsyncMock(return_value=json.dumps(state_data))
                mock_context = AsyncMock()
                mock_context.__aenter__ = AsyncMock(return_value=mock_file)
    ...
    def test_agent_manager_get_agent_statuses(self):
        """Test AgentManager.get_agent_statuses."""
        manager = AgentManager()
        agent1 = Agent(agent_id="agent1", name="Agent 1", description="Test")
        agent2 = Agent(agent_id="agent2", name="Agent 2", description="Test")
