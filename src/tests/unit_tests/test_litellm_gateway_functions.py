...
class TestConvenienceFunctions:
    ...
    def mock_gateway(self):
        ...
        gateway.generate = Mock(return_value=Mock(
            text="Generated text",
            model="gpt-4"
        ))
        ...
        gateway.stream = Mock(return_value=[
            Mock(text="Hello "),
            Mock(text="World")
        ])
        gateway.generate_embeddings = Mock(return_value=Mock(
            embeddings=[[0.1] * 1536, [0.2] * 1536]
        ))
        gateway.generate_embeddings_async = AsyncMock(return_value=Mock(
            embeddings=[[0.1] * 1536]
        ))
        return gateway
...
class TestUtilityFunctions:
    ...
    def mock_gateway(self):
        """Create a mock gateway."""
        gateway = Mock(spec=LiteLLMGateway)
        gateway.generate_async = AsyncMock(side_effect=[
    ...
    async def test_batch_generate_with_exceptions(self, mock_gateway):
        ...
        mock_gateway.generate_async = AsyncMock(side_effect=[
            Mock(text="Response 1"),
            Exception("Error"),
            Mock(text="Response 3")
        ])
