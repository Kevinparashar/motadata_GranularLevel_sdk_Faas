...
class TestOTELGatewayIntegration:
    ...
    def gateway_with_otel(self, mock_otel_tracer, mock_otel_metrics):
        ...
        # TODO: Replace with actual integration when available
    ...
    def test_gateway_metrics_collection(self, gateway_with_otel, mock_otel_metrics):
        ...
        # Record metrics
        gateway._otel_metrics.increment_counter("gateway.requests.queued", {
            "model": "gpt-4",
            "tenant_id": gateway.tenant_id
        })
        
        gateway._otel_metrics.record_histogram("gateway.request.duration", 0.5, {
            "model": "gpt-4"
        })
...
class TestOTELRAGIntegration:
    ...
    def test_rag_metrics_collection(self, rag_with_otel, mock_otel_metrics):
        ...
        rag._otel_metrics.increment_counter("rag.queries.queued", {
            "tenant_id": rag.tenant_id
        })
        
        rag._otel_metrics.record_histogram("rag.query.duration", 1.5, {
            "tenant_id": rag.tenant_id
        })
    ...
    def test_rag_retrieval_performance_tracking(self, rag_with_otel, mock_otel_tracer, mock_otel_metrics):
...
class TestOTELAgentIntegration:
    ...
    def test_agent_task_tracing(self, agent_with_otel, mock_otel_tracer):
        ...
        with agent._otel_tracer.start_trace("agent.task.execute") as trace:
            ...
            with agent._otel_tracer.start_span("agent.memory.retrieve", parent=trace) as memory_span:
    ...
    def test_agent_metrics_collection(self, agent_with_otel, mock_otel_metrics):
        ...
        # Record task metrics
        agent._otel_metrics.increment_counter("agent.tasks.executed", {
            "status": "success"
        })
    ...
    def test_agent_memory_tracking(self, agent_with_otel, mock_otel_tracer, mock_otel_metrics):
        ...
        with agent._otel_tracer.start_span("agent.memory.update") as span:
            ...
            agent._otel_metrics.increment_counter("agent.memory.updates", {
...
class TestOTELTracePropagation:
    ...
    def test_trace_propagation_agent_to_gateway(self, mock_otel_tracer):
        ...
        gateway_span = mock_otel_tracer.start_span(
            "gateway.generate",
            parent=agent_trace
        )
