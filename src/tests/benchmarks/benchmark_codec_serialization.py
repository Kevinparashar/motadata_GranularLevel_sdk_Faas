...
# TODO: Import when CODEC integration is available
...
class TestCodecSerializationBenchmarks:
    ...
    def mock_codec(self):
        ...
        codec.create_envelope = Mock(return_value={
            "schema_version": "1.0",
    ...
    def test_agent_message_serialization(self, benchmark_codec, mock_codec):
        ...
        for _ in range(iterations):
            ...
            # Decode
            start = time.time()
            decoded = mock_codec.decode(encoded)
            latency = (time.time() - start) * 1000  # Convert to ms
        ...
        assert encode_stats["p95"] < 1.0, f"Encode P95 latency {encode_stats['p95']}ms exceeds 1ms target"
        assert decode_stats["p95"] < 1.0, f"Decode P95 latency {decode_stats['p95']}ms exceeds 1ms target"
    ...
    def test_rag_payload_encoding(self, benchmark_codec, mock_codec):
        ...
        # Assert performance targets
        assert encode_stats["p95"] < 2.0, f"Encode P95 latency {encode_stats['p95']}ms exceeds 2ms target"
        assert decode_stats["p95"] < 2.0, f"Decode P95 latency {decode_stats['p95']}ms exceeds 2ms target"
