...
import re
from .prompt_manager import PromptTemplate, PromptStore
...
class FallbackTemplateManager:
    ...
    def get_template_with_fallback(
        ...
        if template:
            ...
            if template_name in self.fallbacks:
                fallback_config = self.fallbacks[template_name]
                if fallback_config.condition:
                    try:
        ...
        if template_name in self.fallbacks:
            fallback_config = self.fallbacks[template_name]
            fallback_template = self.prompt_store.get(
                fallback_config.fallback_template,
                tenant_id=tenant_id,
                version=version
            )
