...
class FeedbackLoop:
    ...
    def register_callback(
        self, feedback_type: FeedbackType, callback: Callable[[FeedbackItem], None]
    ) -> None:
        """
    ...
    def get_feedback_stats(self, tenant_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        stats: Dict[str, Any] = {
            ...
            "pending": len(
                [f for f in self.feedback_queue if not tenant_id or f.tenant_id == tenant_id]
            ),
            "processed": len(
                [f for f in self.processed_feedback if not tenant_id or f.tenant_id == tenant_id]
            ),
            "by_type": {},
        }
