...
class HallucinationDetector:
    ...
    def detect(
        ...
        for sentence in sentences:
            ...
            is_grounded = self._check_sentence_grounding(
                sentence,
                context_text,
                context_documents
            )
        ...
        if self.enable_llm_verification and self.gateway and is_hallucination:
            llm_verification = self._llm_verify_hallucination(
                response,
                context_text,
                query
            )
    ...
    def _check_semantic_similarity(self, sentence: str, context: str) -> float:
        ...
        try:
            ...
            context_embedding = self.gateway.embed(context[:1000], model="text-embedding-3-small")  # Limit context
