...
async def example_leader_follower():
    ...
    leader = Agent(
        agent_id="leader",
        name="Leader Agent",
        description="Leads the coordination"
    )
    ...
    result = await orchestrator.coordinate_leader_follower(
        ...
        leader_task={
            "task_type": "plan",
            "parameters": {"objective": "Complete project"}
        },
        follower_task_template={
            "task_type": "execute",
...
async def example_peer_to_peer():
    ...
    result = await orchestrator.coordinate_peer_to_peer(
        ...
        task_template={
            ...
            "parameters": {"shared_goal": "solve problem"}
        },
        coordination_func=lambda results: {
            "consensus": "achieved" if len(set(str(r) for r in results)) == 1 else "pending"
        }
    )
