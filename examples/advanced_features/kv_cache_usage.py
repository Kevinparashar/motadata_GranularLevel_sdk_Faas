...
from src.core.litellm_gateway import KVCacheManager, create_gateway
...
def main():
    ...
    gateway = create_gateway(
        api_keys={"openai": "sk-..."}, enable_kv_cache=True, kv_cache_ttl=3600  # 1 hour
    )

    # Example 1: Generate with KV cache (automatic)
    print("Generating response with KV cache...")
    response1 = gateway.generate(
        prompt="Explain quantum computing in detail", model="gpt-4", max_tokens=1000
    )
    ...
    # Generate cache key
    cache_key = kv_cache.generate_cache_key(prompt="What is machine learning?", model="gpt-4")
