...
def main():
    ...
    try:
        ...
        result = rag.query(query=query, top_k=2, threshold=0.7, max_tokens=200)

        print(f"\nAnswer: {result['answer']}")
        print(f"\nRetrieved {result['num_documents']} documents:")
        for i, doc in enumerate(result["retrieved_documents"], 1):
            print(
                f"  {i}. {doc.get('title', 'Unknown')} (similarity: {doc.get('similarity', 0):.3f})"
            )
        ...
        async def async_query():
            result = await rag.query_async(
                query="What is machine learning?", top_k=1, threshold=0.7
            )
            return result
